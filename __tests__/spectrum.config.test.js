...
describe('spectrum config module', () => {
  ...
  test('handleSpectrumCommand status enforces guild context', async () => {
    const interaction = {
      guildId: null,
      replied: false,
      deferred: false,
  ...
  test('handleSpectrumCommand status reports configuration when present', async () => {
    spectrumConfig.__testables.configCache.set('guild-5', {
      guildId: 'guild-5',
      announceChannelId: 'chan-5',
      forumId: 'forum-5',
      updatedBy: null,
      updatedAt: new Date()
    });
    ...
    expect(interaction.reply).toHaveBeenCalledWith({
      content: expect.stringContaining('Spectrum Watcher status'),
      flags: MessageFlags.Ephemeral
    });
  });

  test('handleSpectrumCommand status reports when unconfigured', async () => {
    const interaction = {
      guildId: 'guild-status-missing',
      replied: false,
      deferred: false,
      options: {
        getSubcommand: () => 'status'
    ...
    const result = await spectrumConfig.handleSpectrumCommand(interaction);
    expect(result).toMatchObject({ action: 'status', config: null });
    expect(interaction.reply).toHaveBeenCalledWith({
